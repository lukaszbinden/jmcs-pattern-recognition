    ...
    public static class Algorithms 
    {
        ...
        public static List<FeatureVector> ToFeatureVectors(this bool[][] word) {
            ...
            for (int i = 0; i < word.Length; i++) {
                ...
                bool current = word[i][0];
                for (int j = 1; j < word[i].Length; j++) {
                    if (word[i][j] != current) {
                        current = !current;
                        vec[Feature.BlackWhiteTransitions]++;
                    }
                }
                
                // Get the upper and lower contour:
                for (int j = 0; j < word[i].Length; j++) {
                    if (word[i][j]) { // lowest black pixel:
                        vec.lowerContour = j; 
                        break;
                    }
                }
                for (int j = word[i].Length - 1; j >= 0; j--) {
                    if (word[i][j]) { // highest black pixel
                        vec.upperContour = j;
                        break;
                    }
                }
                ...
                vec.blackPercent = (int)(100f * blacks / word[i].Length);
        
                // Get the black within LC -> UC interval:
                if (vec.lowerContour != vec.upperContour) {
                    blacks = 0;
                    for (int j = vec.lowerContour; j < vec.upperContour; j++) {
                        if (word[i][j]) {
                            blacks++;
                        }
                    }

                    vec.blackPercentWithinLcUc = (int) (100f * blacks / (vec.upperContour - vec.lowerContour));
                }
                */
