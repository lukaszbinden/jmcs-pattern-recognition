...
namespace PatRec3 {
        ...
        */
        ...
        public static int DTW(List<FeatureVector> fst, List<FeatureVector> snd) {
            ...
            while (i < fst.Count - 1 && j < snd.Count - 1) {
                ...
                int costIJAdvance = (int)(1.3 * FeatureSqDist(fst[i + 1], snd[j + 1]));
    ...
    internal class Program {
		...
		public static List<Id> toSearch;
        ...
        static Program() {
			...
			toSearch = new List<Id>();
			foreach (var line in File.ReadAllLines("../../data/task/keywords.txt"))
			{
				int[] idNums = line
					            .Split(',')
					            .Skip(1).First()
                                .Split('-')
                                .Select(s => Convert.ToInt32(s)).ToArray();
				toSearch.Add(new Id { page = idNums[0], line = idNums[1], word = idNums[2] });
			}
		...
		public static bool EqualTransliteration(string fst, string snd)
        {
            fst = fst.Replace("-", "").Replace("s_pt", "").Replace("s_cm", "").Replace("s_s", "").Replace("s_mi", "")
                .Replace("s_qs", "").Replace("s_qt", "").Replace("s_qo", "");
            snd = snd.Replace("-", "").Replace("s_pt", "").Replace("s_cm", "").Replace("s_s", "").Replace("s_mi", "")
                .Replace("s_qs", "").Replace("s_qt", "").Replace("s_qo", "");
            return fst.ToLower().Equals(snd.ToLower());
        ...
        public static void Main(string[] args)
        {
            ...
            //Dictionary<Id, List<FeatureVector>> testSet = Algorithms.readPages(270, 271, 272, 273, 274, 275, 276, 277, 278, 279);
            //Dictionary<Id, List<FeatureVector>> validSet = Algorithms.readPages(300, 301,302, 303, 304);

			Dictionary<Id, List<FeatureVector>> testSet = Algorithms.readPages(271, 272, 273, 274, 276, 300);
			Dictionary<Id, List<FeatureVector>> validSet = Algorithms.readPages(305, 306, 307, 308, 309);
           ...
           */
			foreach (Id id in toSearch) {

				List<FeatureVector> word;

                if (!testSet.TryGetValue(id, out word))
                {
                    Console.WriteLine($"{id} not found");
                    continue;
                }
                
                var results = Algorithms.FindMostSimilar(word, validSet).ToArray();//.Where(kvp => !kvp.Key.Equals(id));

				/*
                for (int i = 0; i < results.Length; i++)
                {
                    string found;
                    transcription.TryGetValue(results[i].Key, out found);
                    if (EqualTransliteration(found, keyword))
                    {
            ...
            File.WriteAllText("../../outputFile.csv", stringBuilder.ToString());
